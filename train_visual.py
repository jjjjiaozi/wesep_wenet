...
def train(config="/mnt/afs/250010237/my_projects/wesep_wenet/examples/audio/voxceleb2/config/tse_bsrnn_av.yaml", **kwargs):
    ...
    # 你前面方案二已经算出来的两个值（如果你没放在这里，就在这里重新算也行）
    # num_train_mix = _count_csv_partition(configs["train_data"], train_part)
    # num_val_mix   = _count_csv_partition(configs["val_data"],   val_part)

    # 每 epoch 用多少条 mix（0 -> 跑完 train partition）
    cfg_spe = int(configs["dataset_args"].get("sample_num_per_epoch", 0) or 0)
    sample_num_per_epoch = cfg_spe if cfg_spe > 0 else int(num_train_mix)

    # 分布式：每个 rank 看到的数据大约是 1/world_size
    per_rank_train = max(1, sample_num_per_epoch // world_size)
    per_rank_val = max(1, int(num_val_mix) // world_size)
    ...
    def init_lazy_before_ddp(model, device, configs):
        # 如果没有未初始化参数就直接返回
        if not any(isinstance(p, UninitializedParameter) for p in model.parameters()):
            return

        cue_mode = configs.get("dataset_args", {}).get("cue_mode", "both")

        # 尽量从 config 自动拿到 visual npy 的维度（precomputed_dim）
        vc = (
            configs.get("model_args", {})
                .get("tse_model", {})
                .get("visual", {})
                .get("features", {})
                .get("viscontext", {})
        )
        visual_dim = vc.get("precomputed_dim", None)
        if visual_dim is None:
            # 如果你 npy 就是 cv 维度（常见 256），用 cv 兜底
            visual_dim = vc.get("cv", 256)

        model.eval()
        with torch.no_grad():
            # dummy mix: 1~2秒即可，长度只要能过 STFT/BSRNN
            dummy_mix = torch.zeros(1, 32000, device=device)

            dummy_visual = None
            if cue_mode in ("visual", "both"):
                # dummy visual: (B, Tv, D)
                dummy_visual = torch.zeros(1, 50, int(visual_dim), device=device)

            # 注意：你是 visual-only，所以 enroll=None
            _ = model(dummy_mix, enroll=None, visual_feat=dummy_visual)

        model.train()

    # ====== 在 DDP 之前调用（每个 rank 都会执行）======
    #device = torch.device("cuda")
    #init_lazy_before_ddp(model, device, configs)
